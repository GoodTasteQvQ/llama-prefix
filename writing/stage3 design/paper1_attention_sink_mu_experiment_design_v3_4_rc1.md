# Paper 1 Stage 3 v3.4-rc1：结构 Token、`mu` 标定与安全评估有效性审计

协议版本：`v3.4-rc1`  
拟冻结日期：`2026-07-22`  
适用论文：Paper 1 / IEEE TDSC Regular Paper  
唯一基线：`paper1_attention_sink_mu_experiment_design_v3_3_2.md`；全部旧版本与revision brief保留不修改；本文件在外部输入解除阻断后才可进入Freeze A/B准备  
状态：DESIGN-CLOSED / RUN-BLOCKED；P0 closure register 为
`writing/stage3 design/revision/p0_closure_register.md`。不得运行正式实验，
不得把未生成的 manifest、fixture、coverage、power 或数值写入 Freeze。

### 0.1 v3.4-rc1 closure boundary

本版本唯一基线是 v3.3.2。只整合九项已裁决 P0：
`IDENTITY-CKPT`、`IDENTITY-PROMPT`、`IDENTITY-RNG`、`ANCHOR-DERIVATION`、
`RESAMPLING-LOCK`、`SIM-COMPUTE`、`K2V2-SCOPE`、`TWOPHASE-SE`、
`N-GUARANTEE`。不增加 endpoint、model、sample、scenario、phase schedule、
anchor 或 vector family。任何仓库无法证明的外部输入保持
`BLOCKED_EXTERNAL_INPUT`；该状态是停止执行的唯一结果，不是默认值或可由执行者选择的分支。

唯一身份入口为 `protocol/experiment_identity_manifest.json`。它必须在任何
Freeze A/B 前由真实外部输入填充并 dependency-close；缺少 required field、source
hash、revision 或 partition 时，manifest 校验失败且不得生成替代值。
...
K2-U/K2-B Cross-model unsafe/integrity contrast profile
...
- `K2-U/K2-B`：三模型 categorical dose 的 unsafe/broken anchor-wise marginal contrasts，
  key-secondary estimation with one simultaneous CI family；不做global detection、Holm、raw p或power claim；
- `V2-U/V2-B`：random 与 prompt-domain contrastive direction protocols 的 matched Qwen
  anchor-wise contrasts，key-secondary estimation with one simultaneous CI family；不做global
  detection、Holm、raw p或power claim。
...
本版本只关闭九项register中的identity/anchor contract、exact resampling/substream、
simulation compute stop、K2/V2 scope、two-phase SE与N-guarantee语义；不增加模型、layer、
vector family、phase schedule、anchor数、formal confirmation样本或720-item人工样本规模。
Random-family screen固定为原有三模型×三anchors，不保留未定义的extra-range或midpoint搜索。
...
主表或key-secondary表必须报告：same-`c`的actual `alpha/rho` shift；P2-U/P2-B RD、Holm p与family-wise CI；K2/V2预先排序的anchor-wise estimates与simultaneous CI（不报告global p）；四分类profile；raw-`c`与actual-alpha/rho ranking stability/change/uncertainty；rare unsafe的cluster-aware upper CI；固定stochastic相对greedy的difference-of-RD及cluster-aware CI。
...
`K2-U/K2-B`保留既有model x categorical-rho endpoint模型，但只估计预先排序的12个
anchor-wise marginal contrasts（2 endpoints x 2非Qwen模型 x 3 anchors），以一个同步
max-T simultaneous 95% CI family报告；不计算global interaction statistic、raw p、Holm
或power。  
`V2-U/V2-B`保留既有family x categorical-anchor endpoint模型，但只估计预先排序的6个
matched contrasts（2 endpoints x 3 anchors），以一个同步max-T simultaneous 95% CI family
报告；不计算global interaction statistic、raw p、Holm或power。任何CI排除0只能描述
该anchor-wise estimate，不得称global interaction/detection。
...
`rho_A/rho_T/rho_H`不是Stage3 behavior screen搜索结果，也不得从仓库内已可见的
Stage1 behavior curves、`c=0..2` grid、历史`mu`或当前screen反推。它们只能是任何
`D_behavior_screen` output可见前由真实外部输入直接提供并哈希的设计身份：
...
selection_contract_version = exact-triple-v1
rho_A
rho_T
rho_H
rho_A_binary64
...
唯一selection contract是原样采用 manifest 中唯一的 exact decimal/binary triple；
不运行 conversion、optimization、nearest-grid mapping 或 candidate selection。合法性只检查
decimal/binary逐位一致、`finite(rho_A,rho_T,rho_H)`、`0<rho_A<rho_T<rho_H`、
全部source artifacts早于`D_behavior_screen`、不存在behavior outcome或human gold污染，
且引用满足第6.2节dependency-closed hash规则。缺失、引用不闭合、decimal/binary
不一致、nonfinite、顺序/时间失败或outcome contamination任一成立时状态唯一为
`BLOCKED_EXTERNAL_INPUT`并阻断Freeze B；不得补造默认anchor。

仓库当前没有`base_anchor_manifest.json`、合法source closure或exact triple，因此
v3.4-rc1发布时`ANCHOR-DERIVATION=BLOCKED_EXTERNAL_INPUT`。这不是允许执行者选择任意
ordered triple的许可。
...
- `protocol/experiment_identity_manifest.json`的relative path/schema/hash/status；
- checkpoint source repository ID、absolute path、immutable revision/commit、weight SHA256；
...
- Freeze A与`experiment_identity_manifest.json` relative path、schema version与SHA256；
- `base_anchor_manifest.json` path/schema/hash、source closure、decimal/binary/dtype anchor values与合法性状态；
- `simulation_plan.json` path/schema/hash、scenario union、nuisance inputs、generator、candidate ordering、exact outer/inner counts、seed derivation与acceptance rules；
- `simulation_resource_contract.json` path/schema/hash或明确`BLOCKED_EXTERNAL_INPUT`状态及deterministic estimation-only branch；
...
- P2/rendering Holm family固定顺序与raw p-value定义；K2/V2 fixed contrast-member order；
  各family synchronized max-T CI算法、resample count/seed；
...
`base_anchor_manifest.json`必须早于behavior screen；`simulation_plan.json`与
`statistics_backend_manifest.json`必须早于任何coverage simulation和confirmation。identity/
anchor/backend依赖缺失、hash不闭合或implementation fixture失败时不得生成有效Freeze B。
resource contract缺失或容量不足时不得运行coverage simulation，但可在其他依赖全部真实
闭合后生成明确`P=100,V=30,P2=estimation-only`的resource-failed Freeze B；不得写入虚构的
selection、coverage或power artifact/hash。Freeze B不能包含尚未生成的response identities、
predicted classes或实际inclusion probabilities；这些属于下一节append-only sample lock。
...
这些repository ID不是immutable revision。每个entry必须由
`protocol/experiment_identity_manifest.json`提供唯一revision/commit、执行机absolute
path、weight SHA256、tokenizer revision/hash、chat-template完整文本/hash、layers/heads、
formal layer/depth、hook、dtype/backend、padding、generation config和environment。
不得解析default branch或以同名本地目录代替hash。仓库只证明上述repository ID和若干
历史本地路径，没有三个checkpoint revision/weight/tokenizer/template hashes；当前状态为
`IDENTITY-CKPT=BLOCKED_EXTERNAL_INPUT`，缺一即Freeze A non-zero exit。

### 7.2 Random family

model/layer-specific PRNG unit vectors唯一使用Torch 2.7.1 CPU
`torch.Generator().manual_seed(42)`、`torch.randn(num_vectors, hidden_dim,
dtype=float32, generator=g)`，随后按row L2 normalize；任何zero/nonfinite norm使pool
失败，不重抽。每模型基于其冻结checkpoint identity独立生成；跨模型相同seed不是同一
几何向量，vector nested within model。

唯一index partition为`V_screen=0..9`，`V_confirm=10..(9+V)`，其中
`V in {20,25,30}`只取第18节冻结值；两集合严格不重叠，不按outcome/norm重排、替换或
跳号。manifest保存Torch/runtime、API、seed、hidden dim、pool hash、每向量hash、
checkpoint identity和partition。现有Qwen Stage1 pool/manifest只证明历史算法与seed，
不能冒充三模型Stage3 partition；当前整体状态为
`IDENTITY-RNG=BLOCKED_EXTERNAL_INPUT`，直到三模型pool和partition manifests真实生成。
...
任一required cell不通过时不搜索新anchor：Qwen A/T support失败使相应P2 endpoint estimation-only或blocked；任一三模型cell失败使K2 simultaneous-CI family downgraded。实际screen data、denominator和失败原因必须保留。行为率只可作为independent nuisance输入第18节simulation plan。
...
`experiment_identity_manifest.json`中的每个prompt split必须保存dataset/source/version/
immutable revision、license、retrieval timestamp、language/taxonomy、category-registry
path/hash、record IDs/source SHA256、quotas/probabilities、inclusion/exclusion、normalization、
dedup/leakage结果、selected identities和frame SHA256。Primary target为English。

唯一selection algorithm为：先按canonical record SHA256排序；按已冻结category registry
与Hamilton quota产生final per-category counts；每个split使用
`SHA256(UTF8("prompt-frame-v1|42|" + split_id))[0:16]`解释为big-endian unsigned128，
初始化NumPy PCG64子流并在category内无放回抽样。tie按category key再按record SHA256。
`D_second_confirm`与`D_stochastic_robustness`只能从已冻结`D_behavior_confirm`按同一
contract派生，不得复用Stage1 output identities。

仓库内`data/jbb_behaviors_harmful.json`及metadata可证明100-row JBB harmful候选源，
但缺dataset revision、license、retrieval timestamp和冻结category registry；
`data/safe_pairs.json`也缺V2 provenance。因此它们只能作为候选source artifacts，当前
`IDENTITY-PROMPT=BLOCKED_EXTERNAL_INPUT`；缺任一required field时不得Freeze。
...
| identity manifest | missing checkpoint/prompt/RNG/anchor fields cause non-zero exit and no fallback identity |
| random/V2 vectors | Torch 2.7.1 CPU API、finite/norm、pool checksum、partition、sign、split QA通过 |
...
| interaction patterns | dormant K2/V2 registry的reference/order/hash匹配；不得启动power/global-test rows |
| backend manifest | 缺language/package/optimizer/tolerance/likelihood/Webb/RNG任一字段时non-zero failure |
| raw bootstrap p | 仅P2/rendering small centered-studentized distribution的plus-one p、Holm tie和family order匹配手算 |
| max-T family completion | 只使用family共同成功replicates；任一member non-estimable或`R_successful<9,500`即失败 |
...
| two-phase bootstrap | prompt multiplicity与Rao-Wu `g_hi,b`、clip、fixed-scale `SE_hat`、max-T匹配手算 |
| two-phase downgrade | census、`n=1<N`、zero-denominator、nonfinite weight、`SE<1e-8`、9499/9500边界正确降级 |
| V2 partial budget | 五种allowed state的0/150/300/1,950/2,100 logical paths与rematch约束正确 |
| resampling lock | 每个block exact count、master42、canonical substream label与digest匹配表19.8 |
| budget arithmetic | §18.5.1的27/73/162/239行 fit 算术分别得到5,684,040,000、15,367,960,000、34,104,240,000、50,314,280,000 |
...
7. 每个fit固定160,000 draws、master seed42并按第19.8节fit identity派生子流；
   Monte Carlo SE必须`<0.001`，否则立即触发预冻fallback/estimation-only，不加倍、
   不换seed；160,000是existing cap的exact lock，不声称数学保证MCSE一定通过；
...
Primary CI固定使用9,999个synchronized prompt x phase-two SRSWOR replicates。对replicate
`b`，先在frozen harmful prompt-category strata内对full eligible P2 prompt IDs有放回抽取
原stratum prompt数，记prompt multiplicity为`m_(p,b)`；全部full-universe judge rows使用
该multiplicity。

对每个sample-lock disjoint phase-two stratum `h`，令`N_h`为eligible population size、
`n_h`为SRSWOR validation sample size、`f_h=n_h/N_h`、`d_i=1/pi_i=N_h/n_h`。
phase-two error唯一使用Rao-Wu rescaled replicate weights：
...
if 2 <= n_h < N_h:
    ...
    c_(hi,b) = draw count for sampled item i
...
每个phase-two stratum和replicate使用第19.8节独立PCG64子流；两endpoints共享prompt与
phase-two resample identities。每个replicate以这些weights重算`p_judge_full`、Hájek
residual、相同clip、arm probabilities与两个RD。成功replicate共同集合记`S`。对member
`j`唯一SE与studentization为：
...
SE_(j,b)_star = SE_hat_j
T_(j,b)_star = (theta_(j,b)_star - theta_hat_j) / max(SE_hat_j, 1e-8)
```

这是固定尺度bootstrap-t；不得在replicate内再启动nested bootstrap或替换SE。
`max_j(abs(T_(j,b)_star))`的nearest-rank 0.95 quantile为`q_0.95`，two-sided
simultaneous CI为`theta_hat_j +/- q_0.95*SE_hat_j`。9,999次中至少9,500次必须对两
members共同成功。

以下任一成立即two-phase family non-estimable并触发第17.7节estimation-only downgrade：
`N_h>0,n_h=0`或`pi_i=0`；`n_h=1<N_h`（不能传播SRSWOR phase-two variance；
`n_h=N_h=1`为census且`g=1`）；nonfinite/negative replicate weight；任一arm Hájek
denominator为0；`|S|<9500`；`SE_hat_j<1e-8`或nonfinite。不得reroll、换seed、补抽gold
或扩720；保留point estimates与可计算的描述性区间并记录失败原因。

Mandatory vector-aware sensitivity同样使用上述`g_(hi,b)`，另对prompt IDs与vector IDs
分别有放回抽取，row multiplicity为prompt、vector与phase-two multiplicity的乘积；固定
9,999 replicates、相同family/failure规则。它不替代prompt-cluster primary，也不新增
p-value family。
...
三个categorical `rho` anchors必须逐位等于合法`base_anchor_manifest.json`中的A/T/H identifiers；random screen不得移动、替换或重命名。K2直接使用P2选择出的final `P/V`，K2 estimates/CI precision不得反向增加P/V、模型、anchor或vector数。
...
### 15.2 K2-U/K2-B key-secondary estimation

```text
K2-U: model x categorical_rho contrast vector, y=unsafe
K2-B: model x categorical_rho contrast vector, y=broken
```

分别拟合既有模型，但不形成confirmatory global test：
...
以固定canonical order报告12个anchor-wise model contrasts：endpoint按`unsafe,broken`，
非Qwen model按`Llama,Mistral`，anchor按`A,T,H`。每个endpoint的6个对比共享同一
9,999-replicate model/prompt/vector resampling identity，使用一个同步max-T 95% CI family；
CI只量化tested-support estimate，不能反推global interaction/detection。Full multinomial
profile是descriptive complement，不冒充global test。GLMM/Webb/family completion<0.95或
任一member non-estimable时保留point estimate并将该family标为`CI-only/downgraded`，不
rerun、换seed或改model/anchor。
...
### 16.4 V2 matched key-secondary estimation
...
```text
V2-U: family x categorical_anchor contrast vector, y=unsafe
V2-B: family x categorical_anchor contrast vector, y=broken
```

保留既有模型，但不形成confirmatory global test：
...
direction nested within family；shared prompts形成matched support；pair跨anchors重复。按
固定canonical order（endpoint `unsafe,broken`，anchor `L,M,H`）报告6个matched contrasts，
共享9,999-replicate prompt/direction resampling identity并使用一个同步max-T simultaneous
95% CI family。少于30directions/family使用既有Webb wild procedure；Fallback先删
highest-order pair，再prompt/direction multiway Webb；若Webb未通过null-validity、family
completion<0.95或任一member non-estimable，保留point estimate并标
`CI-only/downgraded`，prompt marginal bootstrap只作sensitivity。Model-based probabilities
使用第14.3节积分。CI排除0不称global interaction/detection，不产生raw p、Holm或power claim。
...
在任何P2/K2/V2 confirmation outcome可见前，且在任何coverage simulation启动前，
必须生成并哈希：
...
`simulation_plan.json`保存scenario union、nuisance source paths/schema/hashes、generator parameters、
candidate ordering、`R_outer=10,526`、`R_inner=9,999`、master seed与第19.8节substream
derivation及acceptance rules。`R_outer=10,526`使在共同completion最低0.95时仍有至少
10,000个successful outer datasets，从而MCSE上界`sqrt(.25/10,000)=0.005`；成功数不足
10,000时candidate直接失败，不追加replicates。允许输入仅为independent behavior screening、
external/historical artifacts、blinded pooled prevalence/missingness/technical-failure summaries
和protocol-fixed effect grids；禁止arm-specific confirmation effect、p-value或human gold。

新增但不预生成的前置契约为：

```text
protocol/simulation_resource_contract.json
```

它必须声明可审计的backend/runtime、可用资源，并覆盖第18.5节的exact fit budget；
缺失、hash不闭合或容量不足时，唯一分支为`resource_feasibility_failed`：不启动
coverage/power simulation，不生成P/V/N或coverage/power artifact，P2按既有fallback
固定为`P=100,V=30, estimation-only`（非power-selected），K2/V2仍为CI-only。当前仓库
没有该契约，状态为`SIM-COMPUTE=BLOCKED_EXTERNAL_INPUT`。
...
```text
outer simulation replicates = 10,526
inner formal resampling replicates = 9,999
...
coverage高于nominal不构成失败；不设upper rejection bound。MCSE按
`sqrt(p_hat*(1-p_hat)/R_successful)`计算。`R_outer`与`R_inner`在artifact中一次冻结，
不按结果增减；任一标准失败即该method/candidate失败，不追加有利scenario或改阈值。
...
candidate必须同时满足automated P2-B Holm zero-null rejection probability `>=0.80`和
automated P2-U mean synchronized family-wise CI half-width `<=0.05`；选择列表中首个
通过者。这里的80%是唯一`automated P2 sub-gate`保证，不覆盖automated max-T、
two-phase或第17.7节完整Boolean gate。其他scenarios只作operating-characteristic
disclosure/method stress。若全部失败，或第18.5.1节resource contract缺失/不足，final
`P=100,V=30`；分别将P2-B标`automated-sub-gate-power-limited/estimation-only`、P2-U
标`precision-limited/estimation-only`。若无有效method，整个P2 family estimation-only。
K2直接使用P2 final P/V，V2固定30 prompts/20 directions；两者不宣称power且不得反向
扩P/V、models、anchors或directions。
...
### 18.5.1 SIM-COMPUTE exact budget and deterministic stop

active P2 scenario rows不改变v3.3.2的union：N-screen为
`9 candidates x 3 q x 3 d x 2 failure = 162`；selected-P/V disclosure union为
`35 prevalence/effect + 33 variance-stress + 6 correlation-stress + 3 failure-stress
= 77`；raw active union `S=239`。core-null acceptance rows为
`54 N-screen null + 19 selected-P/V null = 73`；审计最低null子集（只取`f_screen=0`且
`d=0`）为`27`。

令`R_o=10,526`、`R_i=9,999`、`F=2`（P2-U/P2-B endpoint model fits）。每个scenario
row的outer datasets、inner family resamples和endpoint model-fit budget唯一为：

```text
outer datasets       = S * R_o
inner resamples      = S * R_o * R_i
endpoint model fits  = S * R_o * F * (R_i + 1)
```

| active rows | outer datasets | inner resamples | endpoint fits (observed + inner) |
|---:|---:|---:|---:|
| 27 audit lower bound | 284,202 | 2,841,735,798 | 5,684,040,000 |
| 73 core-null | 768,398 | 7,683,211,602 | 15,367,960,000 |
| 162 N-screen | 1,705,212 | 17,050,414,788 | 34,104,240,000 |
| 239 full active P2 union | 2,516,714 | 25,154,624,286 | 50,314,280,000 |

每个fit最多还需160,000个population-average integration draws；上述fit数尚未计
reduced-GLMM、Webb fallback、technical retry或这些draw operations。K2/V2的既有
52+25 pattern rows不进入active simulation budget，因为它们在本版本预先固定为
key-secondary estimation-only。`simulation_resource_contract.json`至少必须覆盖73-row
core-null的15,367,960,000 endpoint fits；否则按`resource_feasibility_failed`分支，
不得声称任何coverage、power或P/V selection数值。

### 18.6 K2/V2 dormant interaction-pattern registry
...
K2以Qwen为fixed reference，V2以random family为reference；pattern及顺序保留用于说明既有
estimand，不启动K2/V2 power、coverage、minimum-detectable-effect或method-selection
simulation。v3.4-rc1已将两family预先固定为estimation with simultaneous CI；因此这些
registry rows不进入`simulation_plan.json` active workload，不改变P/V，也不得触发模型、
anchor或direction扩张。任何未来激活都需要新amendment，不得冒充本rc1预注册结果。

### 18.7 Detection、practical claim与fixed secondary边界

P2-B报告automated Holm zero-null rejection probability at true RD
`0.05/0.10/0.15`；P2-U报告automated synchronized family-wise CI half-width。
另报`Pr(family-wise lower CI>m_B)` at true RD `0.10/0.15/0.20/0.25`，但这些
都不是完整§17.7 Boolean gate的power。80%只适用于canonical planning scenario中
automated P2-B Holm zero-null rejection sub-gate；不保证automated max-T排除0、two-phase
CI/sign、judge quality/support、positivity/ESS、practical threshold或joint profile。

K2/V2不运行power/global-test simulation，固定为estimation with simultaneous CI；既有
GLMM/Webb顺序只决定CI计算能否执行，失败即`CI-only/downgraded`。Stochastic 1,200条
保持fixed CI-only，不根据observed difference扩样，不进入P2 power/event counts。所有
final P/V、method、generator/backend/scenario hashes和downgrade写Freeze B；默认不作equivalence。
...
| K2-U/K2-B | key-secondary estimation；12-member max-T simultaneous CI；无global p/Holm/power |
| V2-U/V2-B | key-secondary estimation；6-member max-T simultaneous CI；无global p/Holm/power |
...
P2/K2/V2所有model-based marginal probability/RD采用第14.3节random-effect integration：
同draw用于counterfactual arms，每fit固定160,000 draws、master seed42与第19.8节fit
identity子流；MCSE不满足即预冻fallback/estimation-only。不能在不同blocks混用random
effects设0、BLUP平均或conditional logits。
...
Confirmatory p-value family顺序只包括P2=`[P2-U,P2-B]`与
rendering=`[B-A,D-C]`。其GLMM parametric bootstrap与Webb fallback的endpoint raw
two-sided p统一来自预注册centered studentized bootstrap distribution：
...
不得混用Wald、likelihood-ratio或其他bootstrap p。Holm按`p_raw_j`升序，tie按上述
family顺序；同时报告raw与adjusted p。K2/V2不计算`p_raw_j`或Holm；其CI member
order分别固定为第15.2节12-member与第16.4节6-member canonical order。

max-T使用9,999个同步replicates、master seed42与第19.8节canonical family/unit/
replicate substream。每个replicate对family所有成员共享同一prompt/vector或prompt/direction
resampling identities、同一endpoint-correlation draw identity，并完整重拟合/重算
standardization。只有family全部members成功的replicate进入distribution，`R_successful`
对family共同计数。定义`T_j_observed=theta_j_hat/max(SE_j_hat,1e-8)`、
`T_j_star=(theta_j^* - theta_j_hat)/max(SE_j^*,1e-8)`；若原始
`SE_j_hat<1e-8`，该member non-estimable并使procedure失败。取
`max_j(abs(T_j_star))`的nearest-rank 0.95 quantile；two-sided CI为
`theta_j_hat +/- q_0.95*SE_j_hat`。

只丢弃预定义technical fit failures，不按统计值删除replicate；family共同
`R_successful<9,500`或任一原始member non-estimable时procedure失败并进入固定fallback。
Freeze B保存resampling unit、family order、`R_successful`、seed、quantile实现、失败计数和
fixture hash。P2 Holm与automated max-T discordant时固定降为estimation并使用第17.7节
措辞，不用一个替代另一个。

P2 primary method由第18节唯一选择函数在confirmation前确定；confirmation后不得按
显著性、点估计或最有利CI切换GLMM/Webb。K2/V2固定使用同一预注册GLMM→reduced
GLMM→Webb次序计算estimates/CIs，任一路径不合格即family downgrade，不形成检测判定。
Population-average model-based结果统一使用第14.3节random-effect integration；fallback
结果明确标为fixed-tested-conditions或estimation-only。

### 19.7 Detection与estimation边界

K2/V2没有global detection层级。12-member K2与6-member V2 simultaneous CIs只用于
量化预注册anchor-wise estimates；单个或多个CI排除0均不得称global interaction、global
detection或family rejection。method/fit/completion/non-estimable gate失败时整个family保留
point estimates并标`CI-only/downgraded`。Rendering `B-A/D-C`仍使用Holm detection/
max-T localization层级，不得以local CI替代family decision。

### 19.8 RESAMPLING-LOCK

所有required bootstrap/simulation使用master seed42，exact counts如下；不得在Freeze中
选择更大/更小数或用同名未列出的bootstrap替换：

| block_id | exact replicates/draws | role |
|---|---:|---|
| `p1-inference` | 10,000 | stratified prompt bootstrap；all-layer bands复用同一replicates |
| `p1-planning` | 10,000 per registered cell | prospective simulation |
| `e3-ranking` | 10,000 | cluster bootstrap ordering support |
| `p2-raw-webb` | 9,999 | synchronized prompt/vector Webb CI |
| `p2-automated-maxT` | 9,999 | automated P2 Holm raw p and simultaneous CI |
| `p2-two-phase-prompt` | 9,999 | §14.4 prompt x SRSWOR family CI |
| `p2-two-phase-pigeonhole` | 9,999 | vector-aware sensitivity |
| `k1-profile` | 10,000 | global multinomial/profile bootstrap |
| `k2-simultaneous` | 9,999 | 12-member estimation CI only |
| `v2-simultaneous` | 9,999 | 6-member estimation CI only |
| `rendering-maxT` | 9,999 | B-A/D-C family |
| `clean-prompt` | 10,000 | vector-free clean CI |
| `v2-prompt-sensitivity` | 10,000 | fallback transparency sensitivity only |
| `stochastic-two-way` | 10,000 | prompt/vector CI |
| `p2-outer-simulation` | 10,526 per active scenario row | §18.5 outer datasets |
| `p2-inner-simulation` | 9,999 per outer dataset | exact formal family procedure |
| `population-integration` | 160,000 per fit | shared counterfactual random-effect draws |

每个randomized unit独立派生substream，避免parallel scheduling改变结果：

```text
seed_bytes = SHA256(UTF8(
  "v3.4-rc1|master=42|block=" + block_id
  + "|unit=" + canonical_unit_id
  + "|rep=" + zero_padded_decimal_replicate_index
)).digest()[0:16]

subseed = int.from_bytes(seed_bytes, byteorder="big", signed=false)
rng = NumPy Generator(PCG64(subseed))
```

replicate index从1开始；`canonical_unit_id`必须包含family/scenario/candidate/fit identity中
适用字段及hash。同步family members使用相同block/unit/replicate identity；不同block不得
共享stream。Webb weights、prompt/vector multiplicities、phase-two Rao-Wu counts、binary
endpoint draws和integration draws都由相应block substream生成。manifest保存exact label
bytes、前16-byte digest、library/runtime和fixture hash。任何required block未列count、seed、
unit或substream时对应Freeze non-zero exit；fixture未生成时标
`FIXTURE_PENDING_IMPLEMENTATION`，不得写pass。
...
N_v3_4_rc1_logical_automated_judged =
    N_random_logical_automated_judged
  + S_V2
  + C_V2
  + N_V2_random_rematch
...
N_generation_calls =
    N_v3_4_rc1_logical_generated + R_generation_technical

N_automated_judge_calls =
  (N_v3_4_rc1_logical_automated_judged - E_prejudge_terminal)
  + R_judge_parse

N_automated_judge_calls_scheduled_upper =
  N_v3_4_rc1_logical_automated_judged + R_judge_parse

0 <= R_generation_technical <= N_v3_4_rc1_logical_generated
0 <= R_judge_parse <= N_v3_4_rc1_logical_automated_judged
0 <= E_prejudge_terminal <= N_v3_4_rc1_logical_automated_judged
```
...
| method simulation | pre-B manifests/resource contract | P2 fixed scenario union/9 P-V candidates | exact run or resource-failed branch | missing capacity=>P2 estimation-only, no artifact values |
| E5/P2/K1 | behavior/B | 4 logical cells | frozen P/V | primary/key secondary |
| E6/K2 | behavior/B | 3models×3anchors | complete/downgrade | 12-member simultaneous CI only |
| rendering | screen+confirm/B | rescue0/4; A-D | all/partial | secondary |
| benign | benign/B | clean+transition | all models | secondary |
| V2 matched | second/B | 2families×3anchors；P=30/V=20 fixed | complete/downgrade | 6-member simultaneous CI only |
...
- current checkpoint revisions/hashes、prompt frame/category registry与三模型Stage3 vector partitions是外部输入；仓库缺失时协议停止而非猜测；
...
- exact P2 nested validation至少需要15,367,960,000个core-null endpoint fits；无真实resource contract时P2只能estimation-only；
...
```text
results/paper1_stage3_attention_mu_v3_4_rc1/
  protocol/
  ...
  qa/
    ...
    two_phase_srswor_fixtures/
    resampling_lock_fixtures/
    max_t_reference/
...
以上仅是未来artifact contract；本次revision不创建这些目录、manifest、fixture output、
coverage、power或experiment result。当前缺失项保留`BLOCKED_EXTERNAL_INPUT`或
...
7. K2-U/B cross-model categorical estimates with simultaneous CIs；
8. V2-U/B matched random/contrastive estimates with simultaneous CIs；
...
- K2/V2预排序anchor-wise estimates及simultaneous CIs；CI不升级为global interaction；
...
- 对K2/V2任一anchor-wise CI排除0后称global interaction或global detection；
...
- 声称K2/V2存在power或用不存在的K2/V2 power反向增加P/V、model、anchor或direction；
...
- 用K2/V2单anchor或simultaneous CI声称global interaction/global detection；
...
P7  write/hash simulation/backend/resource contracts；若资源可行则按exact outer/inner运行P2 coverage并select P/V/method；否则固定P2 estimation-only
P8  write/hash Freeze B
P9  P2/K1 + E6 K2 CI-only + benign + V2 CI-only
...
- [ ] 所有旧版设计、brief、代码、配置、数据和artifacts未修改；v3.3.2保持唯一基线
- [ ] 协议版本为`v3.4-rc1`；状态为DESIGN-CLOSED/RUN-BLOCKED，未伪装frozen
- [ ] 九项P0 closure register均有evidence/decision/location/fixture/status
- [ ] identity manifest缺checkpoint revision/hash、prompt provenance、RNG partition或anchor exact triple即BLOCKED_EXTERNAL_INPUT
...
- [ ] P2 automated detection sub-gate power与claim probability边界分别记录；不得把sub-gate当完整Boolean power
- [ ] P2-U无practical margin/material wording
- [ ] K2-U/K2-B只报告12-member anchor-wise estimates与simultaneous CIs；不生成Holm/power
- [ ] `base_anchor_manifest.json` exact decimal/binary triple、时点、合法性和block规则完整；无conversion/默认值
...
- [ ] V2-U/V2-B固定6-member CI order、model/fallback与CI-only downgrade；不生成Holm/power
...
- [ ] simulation/backend/resource contracts与estimation-only resource-failed branch唯一
...
- [ ] K2/V2 dormant pattern registry、reference与顺序固定且不进入active simulation
- [ ] P/V candidate顺序及canonical planning scenario唯一
- [ ] automated P2-B sub-gate power与automated P2-U precision联合通过才选择首个candidate
...
- [ ] 仅P2/rendering raw p使用plus-one centered-studentized bootstrap p；K2/V2无raw p
- [ ] max-T只使用family共同成功replicates，9,999中`R_successful<9,500`失败
- [ ] P2 Boolean gate同时含P1/Holm/automated CI/two-phase CI/sign/quality/support
- [ ] Holm/max-T/two-phase冲突一律降级，不按有利结果选择
- [ ] rendering保留global detection/localization；K2/V2明确CI-only、不存在global detection
- [ ] primary仍greedy512
...
- [ ] prompt x Rao-Wu SRSWOR replicate weights、fixed-scale SE/studentization、non-estimable/downgrade可编码
- [ ] two-way pigeonhole sensitivity复用同一phase-two replicate weights且固定9,999
...
- [ ] E0包含identity、base anchor、generator、selector、backend、resampling、Rao-Wu/SE、raw p、quota、Hájek、partial budget与section-reference fixtures
- [ ] RESAMPLING-LOCK exact count/master42/substream labels覆盖所有required bootstrap/simulation
- [ ] SIM-COMPUTE复算27/73/162/239 rows及5,684,040,000/15,367,960,000/34,104,240,000/50,314,280,000 fits
- [ ] 无resource contract时不生成coverage/power/P-V artifact并固定P2 estimation-only
...
```text
Priority 0: checkpoint/prompt/RNG/anchor external inputs + identity manifest + E0
...
Priority 5: simulation/backend/resource contracts；exact P2 coverage or deterministic estimation-only fallback + Freeze B
Priority 6: P2/K1 + K2 CI-only + benign + matched V2 CI-only

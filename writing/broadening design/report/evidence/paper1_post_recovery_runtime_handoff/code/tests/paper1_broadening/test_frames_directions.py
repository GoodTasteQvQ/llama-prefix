...
def test_dual_codex_semantic_review_is_provenanced_and_conservatively_resolved() -> None:
    ...
    decisions = {
        f"safe-pair:{index}": {
            **base,
            "reviewer_a_raw_output": json.dumps({"pair_id": f"safe-pair:{index}", "prompt_revision": base["review_prompt_revision"], "verdict": "include"}),
            "reviewer_b_raw_output": json.dumps({"pair_id": f"safe-pair:{index}", "prompt_revision": base["review_prompt_revision"], "verdict": "include"}),
            "reviewer_a": "include", "reviewer_b": "include", "decision": "include",
        }
        for index in range(500)
    }
    decisions["safe-pair:3"] = {
        **base,
        "reviewer_a_raw_output": json.dumps({
            "pair_id": "safe-pair:3",
            "prompt_revision": base["review_prompt_revision"],
            "verdict": "include",
        }),
...
def test_public_semantic_quality_revision_is_accepted_with_same_gate() -> None:
    pairs = [{"harmful": f"harmful request {index}", "harmless": f"harmless request {index}"} for index in range(416)]
    base = {
        "review_mode": "dual_codex_subagents_v1",
        "review_prompt_revision": "public-semantic-pair-quality-v1",
        "adjudication_rule": "unanimous_include_else_exclude",
        "evaluation_frame_digest": "fixture-evaluation-digest",
        "reviewer_a_agent_id": "agent-a-public",
        "reviewer_b_agent_id": "agent-b-public",
        "reviewer_a_model": "fixture-model-a",
        "reviewer_b_model": "fixture-model-b",
        "reviewer_a_raw_output": '{"verdict":"include"}',
        "reviewer_b_raw_output": '{"verdict":"include"}',
        "reviewer_a_rationale": "materially unsafe versus benign task",
        "reviewer_b_rationale": "materially unsafe versus benign task",
        "reviewed_at": "2026-09-06T00:00:00Z",
    }
    decisions = {
        f"safe-pair:{index}": {

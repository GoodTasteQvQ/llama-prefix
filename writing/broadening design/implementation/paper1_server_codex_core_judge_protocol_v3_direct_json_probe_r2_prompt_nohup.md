# Core Judge protocol v3 direct-JSON compatibility probe (R2 repaired contract)
...
budget-repair Phase 0 已完成 direct-JSON 契约与导入修复，报告状态为
`DIRECT_JSON_IMPLEMENTATION_OFFLINE_PASS` / `READY_FOR_DIRECT_JSON_PROBE_APPROVAL`。
服务器 Phase 0 已验证的代码哈希为：

| 文件 | SHA256 |
|---|---|
| `paper1_broadening/judge.py` | `3bd392cd8ab483b21a1b5566718ffd9396325ee55f8b493a2d8181993ce97d3c` |
| `paper1_broadening/config.py` | `db456b93759336dff1c736458ada53fddad2520fea34bc99b7199b0268a5ae07` |
...
| `tests/paper1_broadening/test_judge_direct_json_protocol.py` | `f431430225042075e9114127991c7f2fae227a97f701882a883e6b5b8d693076` |
| `scripts/paper1_broadening.py` | `80b4d2e2b0578aff331bdd64aa203eca4e1e1871c81a5e2c52eeba7933dfa73f` |
| `scripts/run_paper1_broadening_single_gpu.sh` | `b17afa1c39035ea92d8bbfa8533f8518720d1f0c626e3d48ef3f305a1bf62c14` |
| `tests/paper1_broadening/test_run_readiness.py` | `2cb8f79357284c46822971e6600fc58f4e5293d89357e0b1f02981f82a0a7ff6` |

## 不可违反的边界

1. 先验证固定路径 `.codex-temp/paper1_core_judge_protocol_v3/DIRECT_JSON_PROBE_APPROVAL_R2.md` 存在、状态为 `DIRECT_JSON_PROBE_APPROVED`，并记录批准文件 SHA256。批准文件不完整或哈希不匹配时，不加载模型、不发送请求。
2. 服务器代码哈希必须与上表完全一致。任一不一致时写 `DIRECT_JSON_PROBE_CODE_HASH_MISMATCH` 并停止。R2 不得使用旧 R1 批准或旧 R1 run。
...
4. 只改变 Judge 输出协议：`enable_thinking=false`、`max_new_tokens=1296`、`strict_direct_json_v1`。budget-repair Phase 0 已在离线 fake runtime 中验证该契约；本次 R2 probe 将预算固定为 1296。不得改变模型、endpoint、tokenizer/chat template、dtype、greedy/beam、rubric、label domain、behavior decoder、generation retry 规则或 2% missing gate。
...
创建独立目录：
`results/paper1_broadening/core-judge-protocol-v3-direct-json-probe-r2-<UTC>-<shortid>/`。
...
`DIRECT_JSON_PROBE_FAIL` 和 `DESIGN_CHANGE_REQUIRED`，立即停止，不提高到 1296
以上，不切回 thinking 模式，不补发剩余请求。

最终报告保存至：
`writing/broadening design/report/paper1_core_judge_protocol_v3_direct_json_r2_report.md`。

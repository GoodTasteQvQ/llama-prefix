...
源设计：v1、v2、v3、v3.1、v3.2、v3.3、v3.3.1 与全部 revision brief 保留不修改；本文件是可独立冻结和执行的完整协议  
...
本版本只关闭base-anchor provenance、simulation/backend唯一实现、two-phase positivity与预算审计缺口；不增加模型、layer、vector family、phase schedule、P2 anchor、formal confirmation样本或720-item人工样本规模。random screening固定为原有三模型×三anchors，不再保留未定义的Qwen extra-range或midpoint search。
...
two-phase correction还依赖human-sample positivity、可计算inclusion probability、足够prompt/vector cluster support与非退化analysis weights。任一P2 eligible stratum零抽样概率或cluster/weight gate失败时，human-corrected population inference不可识别，必须降级而不能reroll、换seed或扩720。
...
- `protocol/base_anchor_manifest.json` path/schema/hash、source dependencies与legality result；
- `simulation_plan.json`与`statistics_backend_manifest.json` path/schema/hash、implementation-fixture status；
...
- base `rho_A/rho_T/rho_H` exact decimal/binary values、Qwen `c_A/c_T`、各模型dtype后actual alpha、per-cell support denominator/completion/pass flags及P2/K2 downgrade；
- V2 initial/rescue decision、final `rho_V2_L/M/H`、matched-random reuse/rematch state；
- P2/K2/V2 endpoints、fixed candidate order、selected P/V trace、full/reduced models、method-selection顺序与coverage-acceptance artifact；
...
- fixed720 disjoint-stratum quota algorithm、exact inclusion probability、positivity/cluster/weight gates、Hájek two-phase correction与decision-consistency rule；
- attention target/query/control/missing rules；
- 每个secondary family的multiplicity或CI-only状态；
- complete canonical cell-identity schema、logical/completed/actual-call预算公式及实际代入值、code/config/environment hash、timestamp与自身SHA256。
...
在全部eligible formal greedy responses与locked automated predictions生成后、任何human gold label可见前，由Freeze B算法确定性生成。包含Freeze A/B paths、schema versions与hashes、eligible-universe hash、locked judge/config hash、canonical disjoint-stratum definition、每stratum eligible `N_h`与final `n_h`、quota/critical-constraint trace、selected identities、每item `pi_i=n_h/N_h`、duplicate/eligibility audit、per-cell unique prompt/vector counts、per-arm Kish ESS与maximum normalized weight、positivity和cluster/weight downgrade state、seed、code/config hash、timestamp和按第6.2节计算的自身SHA256。
...
Base anchors不是Stage3 behavior screen的输出。在任何`D_behavior_screen` output可见前，必须创建并按第6.2节哈希：
...
合法性条件唯一为：`rho_A/rho_T/rho_H`全部finite、`0<rho_A<rho_T<rho_H`、全部source artifacts早于`D_behavior_screen`、source fields不含Stage3 confirmation outcomes或human gold，且所有引用满足dependency-closed hash rule。manifest缺失、引用不闭合、数值非法、顺序不严格或来源污染时Freeze B阻断；不得补造默认anchor。

`A/T/H`只作为预注册tested-point identifiers：A是P2-U使用的较低identifier，T是P2-B使用的中间identifier，H是较高identifier；它们不证明unsafe optimum、真实transition或collapse threshold。图表可同时显示中性`L/M/H`，但`c_A/c_T`保留用于endpoint identity。任何attack/transition/collapse-neighborhood措辞必须限定为`preregistered tested-point identifier`，不能写成screen发现的机制位置。

Random-family screen固定为：
...
screen只估计blinded pooled nuisance和检查support，不移动、替换、重命名或重新排序anchors，也不按unsafe/broken/refusal rate选择。每个model-anchor cell只检查：post-dtype alpha finite、hook/layer/config identity complete、retained logical-cell completion rate`>=0.95`、全部retained calls具有finite dose geometry、无unresolved denominator或identity collision。行为率只可输入pre-confirmation simulation plan。

任一required cell不通过时不搜索额外anchor：Qwen A/T失败使对应P2 endpoint estimation-only或blocked；任一K2 common-support cell失败使K2 interaction降为estimation-only；实际screen data、denominator与失败原因必须保留。Freeze B保存manifest path/schema/hash、rho decimal/binary值、各模型dtype后alpha、per-cell denominator/completion/pass flag与downgrade decision。
...
| base-anchor manifest | missing/hash/order/outcome-contamination fixtures均阻止Freeze B，不产生默认anchor |
| random-screen matrix | `3x3x30x10` fixture恰为2,700 logical identities且无extra-range branch |
| marginal generator | zero/random-effect fixtures经bisection后marginal probability误差`<=1e-8` |
| endpoint correlation | fixed synthetic draws的kappa子流、shared clusters与family identities可复现 |
| P/V selector | pass/fail matrix返回candidate list首个联合合格项；全失败返回100/30与endpoint downgrade |
| interaction patterns | K2/V2 registry的reference、pattern order与hash匹配手算列表 |
| statistics backend | incomplete manifest缺optimizer/tolerance/likelihood/Webb/RNG字段时non-zero failure |
| raw bootstrap p | small centered-studentized distribution的plus-one p、Holm tie与family order匹配手算 |
...
| V2 partial budget | 五种allowed states正确产生`0/150/300/1950/2100` paths及rematch约束 |
| budget | planning与scope-grid fixtures分别得到23,034/22,890和59,234/59,090 |
| actual calls | technical retry fixtures将logical/completed/generation attempts/judge attempts正确分列 |
| Freeze hash | canonical JSON fixture可复现external dependency与排除`self_sha256`的自身hash |
| quota construction | sparse/over-target classes的final disjoint quotas、critical constraints与720总数确定性 |
| inclusion probability | known `N_h/n_h`中每item `pi=n_h/N_h`且P2无zero pi |
| cluster/weight gate | concentrated sample触发prompt/vector/ESS/max-weight downgrade且不reroll |
| two-phase RD | hand-computed weighted sample的Hájek residual、clip与RD匹配手算 |
| two-phase bootstrap | fixed prompt multiplicities下full judge与validation residual同步重算 |
...
| max-T family completion | member-failure fixtures只使用family共同成功replicates，completion`<95%`失败 |
| P2 Boolean gate | Holm/automated-CI/two-phase discordant fixtures任一冲突均降级 |
| section references | heading/reference fixture不存在失效的第x.y节引用 |
...
P2-U/P2-B的automated-judge GLMM与raw paired RD保持预注册主要分析。为防止estimator/anchor arms之间的差异性误分类，每个endpoint另计算mandatory human-corrected sensitivity。point estimator固定为self-normalized Hájek residual correction。对endpoint `e`与arm `a`：
...
P2-U使用A identifier的unsafe indicator，P2-B使用T identifier的broken indicator。point estimate与每个bootstrap replicate均执行相同clip。禁止改用unnormalized Horvitz-Thompson、naive human-subset RD、matrix-inversion correction或按结果选择estimator。
...
2. 每个被抽prompt的全部full-universe judge rows继承prompt multiplicity；
3. validation residual rows继承相同prompt multiplicity与固定`1/pi_i`；
4. 每个replicate重算`p_judge_full`、Hájek residual、clip、arm probabilities、RD和两个P2 endpoints；
5. 两endpoints共享同一prompt resample identity；
6. 使用synchronized max-T形成two-phase family-wise CI；
7. 任一arm分母为0的replicate记technical inference failure，family共同成功率必须`>=95%`。

Mandatory vector-aware sensitivity使用two-way pigeonhole bootstrap：prompt IDs与vector IDs分别在各自冻结集合内有放回抽取，row multiplicity为二者multiplicity乘积；其余Hájek、clip、同步family和failure handling与primary相同。它不替代prompt-cluster primary，也不新增p-value family。

最终confirmatory wording执行第17.7节唯一Boolean gate。automated与two-phase不一致、rare gold、positivity、critical-cell capacity、cluster/weight或family completion失败时均降为estimation；不得以overall recall、naive human-subset RD或事后选择的误分类修正替代。

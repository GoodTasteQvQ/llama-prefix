#!/usr/bin/env python3
"""Validate or run the fixed Stage 3 Qwen support screen."""
...
from stage3_pipeline.support_screen import (  # noqa: E402
    DEFAULT_CONFIG,
    run_support_screen,
    validate_only,
)
    ...
    try:
        if args.validate_only:
            ...
            if args.run_mode != "paper":
                raise PipelineError("support-screen execution requires --run-mode paper")
            if args.run_id is None:
                raise PipelineError("--run-mode paper requires an explicit --run-id")
            result = run_support_screen(
                args.config, run_mode=args.run_mode, run_id=args.run_id
            )
    except (OSError, PipelineError) as exc:
        print(json.dumps({
            "status": "SUPPORT_SCREEN_RUNNER_FAIL",
            "error": str(exc),

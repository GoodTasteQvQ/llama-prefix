#!/usr/bin/env python3
"""Validate or run the amended independent formal T/H P2 stage."""
...
from stage3_pipeline.p2_independent_runner import (  # noqa: E402
    DEFAULT_CONFIG,
    run_independent_p2,
    validate_only,
)
    ...
    try:
        if args.validate_only:
            ...
            if args.run_mode != "paper":
                raise PipelineError("independent P2 execution requires --run-mode paper")
            if args.run_id is None:
                raise PipelineError("--run-mode paper requires an explicit --run-id")
            result = run_independent_p2(
                args.config, run_mode=args.run_mode, run_id=args.run_id
            )
    except (OSError, PipelineError) as exc:
        print(json.dumps({
            "status": "P2_INDEPENDENT_TH_RUNNER_FAIL",
            "error": str(exc),
            "validate_only": bool(args.validate_only),
            "model_weights_loaded": False if args.validate_only else None,
            "gpu_used": False if args.validate_only else None,

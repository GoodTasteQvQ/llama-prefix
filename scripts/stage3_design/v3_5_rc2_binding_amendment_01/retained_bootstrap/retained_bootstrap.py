#!/usr/bin/env python3
"""Normative retained-bootstrap reference for v3.5-rc2 amendment 01.

This standard-library-only module evaluates synthetic fixtures or already-frozen
analysis frames. It does not read or generate formal experiment data.
"""
...
VERSION = "v3.5-rc2"
SCHEMA_VERSION = "v3.5-rc2-amendment-01-retained-bootstrap-input-v1"
MASTER_SEED = 42
...
REPORT_STATUS = {
    ...
    "RNG_IDENTITY_INVALID": "REFERENCE_BINDING_FAILURE",
}


class RetainedBootstrapError(ValueError):
    """A first-precedence non-estimability or binding failure."""
    ...
    for cell_id in K1_CELLS:
        ...
        for row in records:
            if row["prompt_id"] not in prompt_position or row["vector_id"] not in vector_position:
                raise RetainedBootstrapError(
                    "INPUT_SCHEMA_INVALID", f"{cell_id} row references identity outside frozen frame"
                )
        ...
        if set(actual_coordinates) != set(expected_coordinates):
            raise RetainedBootstrapError(
                "INPUT_SCHEMA_INVALID",
                f"{cell_id} must contain each frozen prompt/vector coordinate exactly once",
            )
...
def analyze_harmful_clean(
    ...
    for row in records:
        if row["prompt_id"] not in prompt_position:
            raise RetainedBootstrapError(
                "INPUT_SCHEMA_INVALID", "harmful-clean row outside frozen prompt frame"
            )
        if row["label"] not in HARMFUL_LABELS:
...
def evaluate_success_fixture(data: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if data.get("fixture_schema") != "v3.5-rc2-amendment-01-retained-bootstrap-success-input-v1":
    ...
    return {
        "fixture_schema": "v3.5-rc2-amendment-01-retained-bootstrap-success-v1",
        "synthetic_data": True,
...
def evaluate_failure_fixture(
    ...
    if failure_data.get("fixture_schema") != "v3.5-rc2-amendment-01-retained-bootstrap-failure-input-v1":
    ...
    for case_index, raw_case in enumerate(failure_data["cases"]):
        ...
        if mutation == "k1_duplicate_prompt_identity":
            ...
            # Internal failure-contract unit witness. Empty records remain invalid formal input.
    ...
    return {
        "fixture_schema": "v3.5-rc2-amendment-01-retained-bootstrap-failure-v1",
        "synthetic_data": True,
        "results": results,
    }

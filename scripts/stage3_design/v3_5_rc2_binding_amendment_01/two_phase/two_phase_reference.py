#!/usr/bin/env python3
"""Normative matched two-phase inference for v3.5-rc2 amendment 01.

This standard-library-only module fixes the frame, resampling, synchronized RNG,
Rao-Wu replicate-weight, and simultaneous-CI choices left open by the base rc2
binding. It does not run or read any experiment.
"""
...
VERSION = "v3.5-rc2"
AMENDMENT = "binding-amendment-01"
INPUT_SCHEMA = "v3.5-rc2-binding-amendment-01-two-phase-input-v1"
MASTER_SEED = 42
REPLICATES = 9_999
...
INPUT_SCHEMA_PATH = (
    ROOT
    / "writing/stage3 design/v3_5_rc2_binding_amendment_01/two_phase/two_phase_input.schema.json"
)
...
def validate_against_input_schema(payload: Any) -> None:
    ...
    try:
        ...
        raise NonEstimable(
            "REFERENCE_BINDING_FAILURE", "INPUT_SCHEMA_UNAVAILABLE_OR_INVALID", str(exc)
        ) from exc
    if not isinstance(schema, Mapping):
        raise NonEstimable("REFERENCE_BINDING_FAILURE", "INPUT_SCHEMA_NOT_OBJECT")
...
def seed_material(block_id: str, unit_id: str, replicate_index: int) -> tuple[str, bytes]:
    ...
    payload = (
        f"{VERSION}|master={MASTER_SEED}|block={block_id}"
...
def refresh_frame_hashes(payload: dict[str, Any]) -> dict[str, Any]:
    """Fixture-author helper; production callers must supply pre-frozen hashes."""
...
def validate_input(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if payload["data_origin"] not in ("SYNTHETIC_GOLDEN_ONLY", "FORMAL_FROZEN_INPUT"):
        _fail_input("INVALID_DATA_ORIGIN")
    if payload["synthetic_data"] != (payload["data_origin"] == "SYNTHETIC_GOLDEN_ONLY"):
    ...
    if payload["data_origin"] == "FORMAL_FROZEN_INPUT" and len(prompt_category) != 50:
        _fail_input("FORMAL_PROMPT_FRAME_NOT_50", str(len(prompt_category)))
...
def evaluate_success_fixture(fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "fixture_schema": "v3.5-rc2-binding-amendment-01-two-phase-success-v1",
        "synthetic_data": True,
...
def evaluate_failure_fixture(base_fixture: Mapping[str, Any], failure_fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "fixture_schema": "v3.5-rc2-binding-amendment-01-two-phase-failure-v1",
        "synthetic_data": True,
        "results": results,
    }
...
def evaluate_schema_negative_fixture(
    ...
    return {
        "fixture_schema": "v3.5-rc2-binding-amendment-01-two-phase-schema-negative-v1",
        "synthetic_data": True,
        "results": results,
    }
